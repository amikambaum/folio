...
# Remove previous gf block by markers (if קיים)
$text = [regex]::Replace($text, '(?s)//\s*===\s*gf:.*?BEGIN.*?(//\s*===\s*gf:.*?END.*?===)?', '')

# Remove any loose gf_* enums/models (אם הושתלו בלי markers)
do {
  $before = $text
...
# Fresh, valid gf_* block (enums multi-line)
$gfBlock = @'
...
Write-Host "schema.prisma repaired (multiline enums) and gf_* block appended."
